    ...
    public:
        ...
        )
      {
        ...
        while (true)
        {
          ...
          {
            const auto hline = dataGetter::ordinateOf(pl);

            if (hline <= dataGetter::ordinateOf(pointMoreUpper))
            {
              const auto vline = dataGetter::abscissaOf(pointMoreUpper);
              const auto abscissaOfMoreLower = dataGetter::abscissaOf(pointMoreLower);
              if (vline < abscissaOfMoreLower)
              {
                const auto tonLower = getterLower.getTofAbscissa(vline);
              ...
              else if (vline > abscissaOfMoreLower)
              {
                const auto tonUpper = getterUpper.getTofAbscissa(abscissaOfMoreLower);
            ...
            {
              const auto tonUpper = getterUpper.getTofOrdinate(hline);
              const auto ponUpper = getterUpper.getPoint(tonUpper);

              const auto vline = dataGetter::abscissaOf(ponUpper);
              const auto abscissaOfMoreLower = dataGetter::abscissaOf(pointMoreLower);
              if (vline < abscissaOfMoreLower)
              {
                const auto tonLower = getterLower.getTofAbscissa(vline);
              ...
              {
                const auto tonUpperBack = getterUpper.getTofAbscissa(abscissaOfMoreLower);
                const auto ponUpperBack = getterUpper.getPoint(tonUpperBack);

                if (point::isSame(ponUpperBack, pointMoreLower))
                {
                  return std::tuple{ tonUpperBack, tofmaxLower, true };
                }
                else
                {
                  return std::nullopt;
